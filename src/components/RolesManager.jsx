...
export function RolesManager({ roles = [], onAddRole, onUpdateRole, onDeleteRole }) {
  ...
  const [editingRole, setEditingRole] = useState(null); // { id, nombre }
  const [deletingRole, setDeletingRole] = useState(null); // { id, nombre }
  ...
  const handleUpdateRole = () => {
    if (editingRole && editingRole.nombre.trim()) {
      onUpdateRole(editingRole.id, editingRole.nombre.trim());
      setEditingRole(null);
    }
  };
  ...
  return (
    <>
      <Card className="border-0 shadow-none">
        <CardContent className="p-0 pt-4 space-y-2 max-h-[300px] overflow-y-auto">
         {roles.length > 0 ? roles.map((rol) => (
  <div key={rol.id ?? Math.random()} className="flex items-center justify-between p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
    
    {/* --- MODO EDICIÓN --- */}
    {editingRole?.id === rol.id ? (
      <Input
        // El valor se toma del estado 'editingRole'. Si 'nombre' no existe, será un string vacío.
    ...
    <div className="flex items-center gap-1">
      {editingRole?.id === rol.id ? (
        ...
        <>
          <Button variant="ghost" size="icon" className="h-8 w-8 text-success hover:bg-success/10 hover:text-success" onClick={handleUpdateRole}>
        ...
        <>
          <Button 
            ...
            onClick={() => {
              // DEBUG: Revisa la consola para ver la estructura del objeto 'rol'.
              console.log("Iniciando edición para:", rol); 
              setEditingRole({ ...rol });
            }}
          >
  ...
  <p className="text-sm text-center text-muted-foreground py-4">No hay roles definidos.</p>
        ...
        <CardFooter className="pt-6">
          <div className="flex w-full items-center gap-2">
            ...
            />
            <Button onClick={handleAddRole} className="shrink-0" variant="professional">
        ...
        <AlertDialogContent className="bg-card/95 backdrop-blur-sm border-0 shadow-xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              Las personas que tengan este rol quedarán sin asignación.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
